...
def translate_text(text, target_language):
    ...
    if response.status_code == 200:
        ...
        if start_index != -1 and end_index != -1:
            ...
            print(f"WARN: Translation tags not found in the response, Skipping (likely empty text)")
            return text
    else:
...
def translate_group_shapes(group_shape, target_language):
    for shape in group_shape.shapes:
        if shape.has_text_frame:
            ...
            translate_group_shapes(shape, target_language)

def process_shape(shape, target_language):
    if shape.has_text_frame:
        translate_shape_text(shape, target_language)
    elif shape.has_table:
        translate_table(shape.table, target_language)
    ...
    with tqdm(total=slide_count, desc="Translating", unit="slide") as pbar:
        for i, slide in enumerate(input_ppt.slides):
            for shape in slide.shapes:
                try:
                    process_shape(shape, target_language)
                except Exception as e:
